...
## Set Target CPU based on HOST arch
case "$(uname -m)" in
    x86_64|amd64)
        HOST_ARCH="x86_64"
        ;;
...
case "$ARCH" in
    ...
    aarch64|arm64)
        ...
        ;;
    *)
        echo "Unsupported node architecture: $ARCH"
        exit 1
        ;;
esac
...
case "$OS" in
    ...
    Darwin)
        ...
        QEMU_EFI="QEMU_EFI-${ARCH}.fd"
        if [[ ! -f $QEMU_EFI ]] ; then
            if [[ -n "$(which colima)" ]] ; then
                echo "--- using ${QEMU_EFI} from colima"
                colima ssh -- sudo apt-get install --yes qemu-efi-aarch64 ovmf
                colima ssh -- cp -v /usr/share/qemu-efi-aarch64/QEMU_EFI.fd ./QEMU_EFI-aarch64.fd
                colima ssh -- cp -v /usr/share/ovmf/OVMF.fd ./QEMU_EFI-x86_64.fd
            else 
                echo "--- downloading QEMU_EFI-aarch64.fd"
                wget -nv -nc https://releases.linaro.org/components/kernel/uefi-linaro/latest/release/qemu64/QEMU_EFI.fd -O ./QEMU_EFI-aarch64.fd
            fi
        fi
        case "$ARCH" in
            aarch64)
